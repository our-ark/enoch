...
class TelegramVisionError(RuntimeError):
    pass
...
def temporary_telegram_image(
    ...
    if image.file_size > MAX_TELEGRAM_IMAGE_BYTES:
        raise TelegramVisionError("Telegram image is too large.")
...
def telegram_image_prompt(caption: str) -> str:
    ...
    return "\n\n".join(
        [
            ...
            "The attached image came from the configured human's Telegram chat.",
...
def _validate_image(path: Path, suffix: str) -> None:
    try:
        ...
        raise TelegramVisionError("Could not read the Telegram image.") from error
    if size == 0:
        raise TelegramVisionError("Telegram returned an empty image.")
    ...
    if not valid:
        raise TelegramVisionError("Telegram returned an unsupported or invalid image.")
